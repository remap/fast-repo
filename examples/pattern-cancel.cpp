 ...
 int startBlockId = -1, int endBlockId = -1)
{
    ...
    (interest, bind(&requestCancelOnData, _1, _2, onInsertStarted, onFailed),
     bind(&requestCancelOnTimeout, _1, onFailed));
}

static void
requestCancelOnData
(const ptr_lib::shared_ptr<const Interest>& interest,
 const ptr_lib::shared_ptr<Data>& data, const SimpleCallback& onCancelSucceed,
 const SimpleCallback& onFailed)
{
...
(const ptr_lib::shared_ptr<const Interest>& interest,
 const SimpleCallback& onFailed)
{
    cout << "Cancel repo command timeout" << endl;
    onFailed();
}
...
static void
onCancelSucceed(const Name& fetchPrefix);
...
int main(int argc, char** argv)
{
    try {
        ...
        (face, repoCommandPrefix, fetchPrefix,
         bind(&onCancelSucceed, fetchPrefix),
...
onCancelSucceed(const Name& fetchPrefix)
{
    cout << "Cancel succeeded for " << fetchPrefix.toUri() << endl;
    exit(0);
}
